...
inline DWORD WRITE_PT_FREQUENCY_MOVE_CS(BYTE *buffer, float POSX, float POSY, float POSZ, float ANGLEY)
{
	CStreamSP Stream;
	Stream->SetBuffer(buffer);
...
inline DWORD WRITE_PT_FREQUENCY_MOVE_SC(BYTE *buffer,INT SLOT_ID, float POSX, float POSY, float POSZ, float ANGLEY)
{
	CStreamSP Stream;
	Stream->SetBuffer(buffer);
